...
import { Briefcase, Building, MapPin, Clock, Send, AlertCircle, CheckCircle } from "lucide-react"
import Sidebar from "../components/Sidebar"
import Header from "../components/Header"
import { updateJobAPI } from "../../API/jobApi"
import { JobContext } from "../App"

const EditJob = () => {
    const { id } = useParams()
    const jobContext = useContext(JobContext)
    const job = jobContext.data.filter((job) => job._id === id)[0]
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await updateJobAPI(id, formData)
            if (res.data.success) {
                jobContext.updateJob(id, formData)
                navigate("/home")
            }
        } catch (err) {
                            ...
                            >
                                {loading ? (
                                    ...
                                    <>
                                        ...
                                        <span>Update Application Tracker</span>
                                    </>
                                )}
                            </button>
                        </form>
